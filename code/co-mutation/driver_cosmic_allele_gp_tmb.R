...
set.seed(INDEX)
library(here)
outdir <- here("output", "co-mutation", "driver_cosmic_allele_gp_tmb.R")
...
data(driver_cosmic_allele_gp_tmb, package="contingency.table")
mt  <- driver_cosmic_allele_gp_tmb %>%
  unnest(data)  %>%
...
tmb <- fit_stratify("tmb", mt2, params) %>%
  mutate(nonras=mt2$nonras, category="Inactivating TMB")
saveRDS(tmb, file=here(outdir, paste0(gene, ".rds")))
stop("done")
q('no')


mt <- inactivating_allele_tmb %>%
  separate(uid, c("ras", "nonras", "cancer"), sep="~")
gene <- unique(mt$ras)[INDEX]
mt <- mt %>%
  filter(ras==gene)
