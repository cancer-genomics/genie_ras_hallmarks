...
set.seed(INDEX)
library(here)
outdir <- here("output", "co-mutation", "any_amplification.R")
...
data(any_amplification_ct, package="contingency.table")
mt  <- any_amplification_ct %>%
    unnest(data)  %>%
    ##filter(number_ras > 50, number_nonras > 50) %>%
