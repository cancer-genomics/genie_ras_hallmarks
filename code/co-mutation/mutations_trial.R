...
set.seed(INDEX)
library(here)
outdir <- here("output", "co-mutation", "mutations.R")
...
load("/dcs04/scharpf/data/abalan/kras_cd/contingency.table/data/mutation_ct_trial.rda")
mutation_ct <- mutation_ct_trial
mt  <- mutation_ct %>%
    unnest(data)  %>%
