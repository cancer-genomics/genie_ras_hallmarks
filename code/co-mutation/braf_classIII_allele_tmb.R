.libPaths("/users/abalan/Library/R/3.12-bioc-release-conda/")
if(!exists("INDEX")){
...
set.seed(INDEX)
library(here)
outdir <- here("output", "co-mutation", "braf_classIII_allele_tmb.R")
...
data(braf_classIII_allele_tmb, package="contingency.table")
mt  <- braf_classIII_allele_tmb %>%
  unnest(data)  %>%
...
##               stanmodel=model)
tmb <- fit_stratify("tmb", mt2, params) %>%
  mutate(nonras=mt2$nonras, category="TMB")
saveRDS(tmb, file=here(outdir, paste0(gene, ".rds")))
stop("done")
q('no')

data(mutation_allele_tmb, package="contingency.table")
mt <- mutation_allele_tmb %>%
...
fit_stan <- function(x, params) {
  stan.data <- stan_format(x)
  tmp <- sampler(stan.data, params)
  xx <- x %>%
    group_by(cancer) %>%
    nest()
  xx$mcmc <- list(tmp)
...
saveRDS(mt, file=here(outdir, paste0(gene, ".rds")))
q('no')
